...
| 2️⃣ | **Huỳnh Nguyên Bổn** | Lập trình chính | Giao diện chính (`MainWindow`), phát nhạc local, xử lý Play/Pause/Next, thiết kế UI, quản lý cấu trúc project, hiệu ứng chuyển trang.
| 3️⃣ | **Nguyễn Lê Quốc Anh** | Lập trình viên API | Tích hợp Spotify API, gọi API tìm kiếm nhạc, phát bài trên Spotify, xử lý token.
| 4️⃣ | **Phan Nhất Duy** | Lập trình viên UI phụ | Danh sách yêu thích, hiệu ứng giao diện, lưu yêu thích JSON.
